...
私は毎日想像を絶する狂気的な学習・設計・製造を続け、初心者の状態から50回以上の作り直しを経て、衣笠を何とか稼働できるレベルまで仕上げました。
私は本職がSEですが、ゲーム開発会社の社員ではないので、これらは独学で作ったものです。独学で作ることにこそ、意味があったのです。誰かの力を借りて作るなんて誰でもできるんです。

しかし、最近課題が出てきました。私のゲームに必要な要素を実装していった衣笠は、まるでサイコガンダムのように巨大化し（ゲームフレームワークがデカすぎる！）、今では約7万ステップを超え、もはや汎用ゲームエンジンとゲーム本体を切り分けるという当初の設計思想の達成が難しくなっていました。
