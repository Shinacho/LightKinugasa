 ...
 * @author Shinacho<br>
 */
public abstract class ArrayIndexModel extends CloneableObject {
	 ...
	 * したがって、次の呼び出しではアルゴリズムに沿った 「次の値」を返します。<br>
	 *
	 * @param arrayLength 配列の長さ(length)を送信します。<br>
	 *
	 * @return 配列のインデックスを返します。インデックスは0からarrayLength-1である必要があります。<br>
	 */
	public abstract int next(int arrayLength);

	public int next(Collection<?> c) {
		return next(c.size());
	}
