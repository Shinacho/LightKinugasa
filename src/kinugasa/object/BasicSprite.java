...
public abstract class BasicSprite extends Sprite implements Controllable {
	...
	public BasicSprite(float x, float y, float w, float h) {
		this(x, y, w, h, new KVector());
	}

	public BasicSprite(float x, float y, float w, float h, KVector vector) {
		super(x, y, w, h);
		this.vector = vector;
		this.moving = BasicMoving.getInstance();
	}

	public BasicSprite(float x, float y, float w, float h, KVector vector, MovingModel model) {
		super(x, y, w, h);
		this.vector = vector;
		this.moving = model;
	}

	public BasicSprite(float w, float h, KVector vector, MovingModel model) {
		super(0, 0, w, h);
		this.vector = vector;
		this.moving = model;
	}
